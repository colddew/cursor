...
以“广州”为例，从 cities 表中获取其 year、base_min, base_max, 和 rate。
对于每位员工，将其“年度月平均工资”与广州的基数上下限进行比较，以确定“最终缴费基数”。规则是：低于下限用下限，高于上限用上限，在中间则用平均工资本身。
...
你的任务： 请根据以上全部需求，首先生成 claude.md 

只要计划文件。不要生成任何代码。等待我审查并确认计划后，我们再开始具体的编码工作。

复述我的需求，提出你的问题。

-----

1、只从社保系统上传两个excel文件，cities是社保标准，salaries是工资数据
2、目前只提供广州的社保标准，计算这个省的即可。后面要能够保持扩展到其他城市
3、以年为单位，按某一年计算，需要做year字段与YYYYMM格式month的匹配。 year跟YYYY对应上
4、UI简洁大气就可以，社保网站也不用太花哨
5、不要测试数据，完全依赖用户上传

-----

确认没有问题。我们按照todolist开始开发吧。说说你要先开始做哪些工作